...
def test_cli_queue_refresh_writes_tables_to_source_and_archived(base_directory: pathlib.Path) -> None:
    ...
    with mock.patch("dandi_compute_code.queue._pipeline_queue.PipelineQueue.write_dandiset_jobs_table") as mock_write:
        result = runner.invoke(_dandicompute_group, ["queue", "refresh", "--base", str(base_directory)], env=_DANDI_ENV)
...
@pytest.mark.ai_generated
def test_cli_queue_refresh_forwards_custom_dandiset_ids(base_directory: pathlib.Path) -> None:
...
@pytest.mark.ai_generated
def test_cli_queue_refresh_forwards_base_and_test_flags(base_directory: pathlib.Path) -> None:
    """dandicompute queue refresh forwards --base/--test to write_dandiset_jobs_table."""
    runner = CliRunner()
    with mock.patch("dandi_compute_code.queue._pipeline_queue.PipelineQueue.write_dandiset_jobs_table") as mock_write:
        result = runner.invoke(
            _dandicompute_group,
            ["queue", "refresh", "--base", str(base_directory), "--test"],
            env=_DANDI_ENV,
        )
    assert result.exit_code == 0, result.output
...
@pytest.mark.ai_generated
def test_cli_queue_refresh_fails_without_api_key(base_directory: pathlib.Path) -> None:
    ...
    with mock.patch.dict(os.environ, env_without_key, clear=True):
        result = runner.invoke(_dandicompute_group, ["queue", "refresh", "--base", str(base_directory)])
    assert result.exit_code != 0
    assert "DANDI_API_KEY" in result.output


@pytest.mark.ai_generated
def test_cli_queue_refresh_fails_without_dandi_devel(base_directory: pathlib.Path) -> None:
