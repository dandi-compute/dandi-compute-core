...
Code conventions, commit rules and the versioning policy are in [`AGENTS.md`](https://github.com/dandi-compute/dandi-compute-core/blob/main/AGENTS.md) and [`CONTRIBUTING.md`](https://github.com/dandi-compute/dandi-compute-core/blob/main/.github/CONTRIBUTING.md). This page covers the tasks specific to this package.
...
Edit the pipeline's `dispatch` block in `pipeline_configs.json`. Only `max_concurrent` and `max_array_tasks` are configurable. Resource requests come from each capsule's own `submit.sh`, so change them in the pipeline's submission template. See [Array dispatch](dispatch.md).

## Adding a pipeline

The queue is pipeline agnostic, and everything that differs per pipeline is reached through four hooks on {class}`~dandi_compute_code.queue.PipelineQueue`.
...
The base class serves any pipeline no subclass claims, which is how `lfp` works. A pipeline that needs its own behaviour subclasses `PipelineQueue` and names itself in `pipelines`, as {class}`~dandi_compute_code.queue.AindEphysPipelineQueue` does for `aind+ephys`. The subclass must be imported by `queue/__init__.py` for `PipelineQueue.for_pipeline` to find it.

A new pipeline also needs:

- a subpackage with a `prepare_*_job` function that follows the capsule layout and writes the `DandiCompute` provenance block (see [Job capsules](job_capsules.md))
